...
impl<T: ContainerSize, O: BitOrder, S: BitStore> BitContainer<T, O, S> where BitSlice<O, S>: BitField {
    /// If the container is 8 bits or under, it can represent the bit vector as a single byte.
    pub fn as_byte(&self) -> crate::Result<u8> {
        if T::size() > 8 {
            return Err(crate::Error::Message("Tried to convert a bit vector larger then a single byte into a single byte.".to_owned()))
        }
        let mut result = 0u8;
        for i in 0..T::size() {
            if self.0[i] {
...
struct BitContainerVisitor<T: ContainerSize, O: BitOrder, S: BitStore>(PhantomData<T>, PhantomData<O>, PhantomData<S>);
impl<'de, T: ContainerSize, O: BitOrder, S: BitStore> Visitor<'de> for BitContainerVisitor<T, O, S> {
    ...
    fn visit_seq<A>(self, mut seq: A) -> Result<Self::Value, <A as SeqAccess<'de>>::Error> where
        ...
        for _ in 0..T::size() {
            vec.push(seq.next_element::<bool>()?.ok_or(A::Error::custom("Couldn't grab next bit"))?)
        }
